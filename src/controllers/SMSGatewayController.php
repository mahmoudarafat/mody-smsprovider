...
namespace mody\smsgateway\controllers;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use mody\smsgateway\Models\Message;
use mody\smsgateway\Models\Provider;
use mody\smsgateway\Models\Track;
use mody\smsgateway\traits\DatabaseConfig;
use mody\smsgateway\traits\ProviderConfig;
use mody\smsgateway\traits\SMSGateway;
use DB;
...
class SMSGatewayController extends Controller
{
    ...
    public function editProviderConfig($provider_id)
    {
        ...
        return view('smsgateway::edit-provider', compact('provider'));
    }
    ...
    {
        $title = trans('smsgateway::smsgateway.user_providers_title');
        $trytwo = $this->authProviders();
        return view('smsgateway::auth-providers', compact('trytwo', 'title'));
    }
    ...
    {
        $title = trans('smsgateway::smsgateway.group_providers_title');
        $tryone = $this->myGroupProviders();
        return view('smsgateway::group-providers', compact('tryone', 'title'));
    }
    ...
    {
        $title = trans('smsgateway::smsgateway.user_trashed_providers_title');
    ...
    {
        $title = trans('smsgateway::smsgateway.group_trashed_providers_title');
    ...
    public function submitSetup(Request $request)
    {
        ...
        try{
            ...
            return redirect()->route('smsgateway.providers.auth_index')->with([
                'success' => trans('smsgateway::smsgateway.saved')
            ]);
        }catch(\Exception $e){
            return redirect()->back()->with([
                'error' =>  trans('smsgateway::smsgateway.error')
            ]);
        }
    }
