...
@section('content')
    ...
    <div class="container" style="margin-top:3em;">
        <div class="row">
            ...
            <div class="col-md-12">
                <h1 class="pager text-primary"><u class="text-muted">{{ $provider->company_name }}</u> {{ trans('smsgateway::smsgateway.config_title') }}</h1>
                <hr>
            </div>
            <div class="col-md-10 col-md-offset-1">
                <form action="{{ route('smsgateway.providers.update_setup') }}" method="post">
                    {!! csrf_field() !!}
                    <input name="provider_id" value="{{ $provider->id }}" type="hidden">

                    <h2 class="title-page">{{ trans('smsgateway::smsgateway.api_settings') }}</h2>

                    <div class="form-group">
                        <label for="api_company">{{ trans('smsgateway::smsgateway.api_company') }}</label>
                        <input type="text" name="api_company" value="{{ $provider->company_name }}" id="api_company" class="form-control">
                    </div>

                    <div class="form-group">
                        <label for="api_url">{{ trans('smsgateway::smsgateway.api_url') }}</label>
                        <input type="text" name="api_url" value="{{ $provider->api_url }}" id="api_url" class="form-control">
                    </div>

                    <div class="form-group">
                        <label for="api_method">{{ trans('smsgateway::smsgateway.http_method') }}</label>
                    ...
                    <div class="form-group">
                        <label for="api_destination">{{ trans('smsgateway::smsgateway.destination_attr') }}</label>
                        <input type="text" value="{{ $provider->destination_attr }}" name="api_destination" id="api_destination" class="form-control">
                    </div>

                    <div class="form-group">
                        <label for="api_message">{{ trans('smsgateway::smsgateway.message_attr') }}</label>
                        <input type="text" value="{{ $provider->message_attr }}" name="api_message" id="api_message" class="form-control">
                    </div>

                    <div class="form-group">
                        <label for="api_success_code">{{ trans('smsgateway::smsgateway.success_code') }}</label>
                    ...
                    <div class="form-inline">
                        <label for="api_unicode" class="col-md-2"
                               style="margin-top: 0.85em;">{{ trans('smsgateway::smsgateway.use') }} UTF-8؟</label>
                        <input type="checkbox" name="api_unicode" style="width:2em;" id="api_unicode"
                               class="form-control" @if($provider->unicode) checked @endif>
                        {{ trans('smsgateway::smsgateway.utf-8') }}        </div>

                    <hr>
                    <div class="form-group">
                        <h2 class="title-page">{{ trans('smsgateway::smsgateway.add_params') }}</h2>

                        <div class="row">
                            <div class="col-md-8">
                                <a href="javascript:void(0)" id="new_additional"
                                   class="btn btn-info col-md-2">{{ trans('smsgateway::smsgateway.add') }}</a>
                            </div>
                        </div>
                        <div id="additional_inputs">
                            ...
                            <h2 class="title-page">{{ trans('smsgateway::smsgateway.account_settings') }}</h2>

                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="username_column">{{ trans('smsgateway::smsgateway.username_column') }}</label>
                                        <input type="text" name="username_column" id="username_column" class="form-control">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="username_value">{{ trans('smsgateway::smsgateway.username_value') }}</label>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="api_password_column">{{ trans('smsgateway::smsgateway.password_column') }}</label>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        <label for="api_password_value">{{ trans('smsgateway::smsgateway.password_value') }}</label>
                            ...
                            <div class="col-md-7 col-md-offset-1" style="margin-bottom: 1em;">
                                <div class="row" id="add_raw_mody">
                                    <div class="col-md-5">
                                        <div class="form-inline">
                                            <label>{{ trans('smsgateway::smsgateway.parameter') }}</label>
                                    ...
                                    <div class="col-md-5">
                                        <div class="form-inline">
                                            <label>{{ trans('smsgateway::smsgateway.value') }}</label>
                                    ...
                                    <div class="col-md-2">
                                        <a style="margin-top:1.7em;" href="javascript:void(0)"
                                           class="btn btn-danger delete_add_raw" data-id="mody">
                                            {{ trans('smsgateway::smsgateway.delete') }}</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-md-6 col-md-offset-2">
                                <hr>
                                <button type="submit"
                                        class="btn btn-primary">{{ trans('smsgateway::smsgateway.save') }}</button>
                                <button type="reset"
                                        class="btn btn-warning">{{ trans('smsgateway::smsgateway.erase') }} </button>
                            </div>
                        </div>
                    </div>
        ...
        $(document).ready(function () {
            $(document).on('click', '#new_additional', function () {
                ...
                $('#additional_inputs').append('<div class="col-md-7 col-md-offset-1"  style="margin-bottom: 1em;">\n' +
                    ...
                    '                </div>\n' +
                    '                <div class="col-md-2">\n' +
                    '                    <a href="javascript:void(0)" class="btn btn-danger delete_add_raw" data-id="' + random + '">' + '{{ trans('smsgateway::smsgateway.delete') }}' + '</a>\n' +
                    '                </div>\n' +
                    '            </div>\n' +
                    '        </div>\n');
            });
