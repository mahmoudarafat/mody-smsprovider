...
//        $post_body = $this->seven_bit_sms($username, $password, $seven_bit_msg, $msisdn);
    ...
    */
    ...
    protected function sendUnicodeSMS()
    {
//        $post_body = $this->unicode_sms($username, $password, $unicode_msg, $msisdn);
    ...
    */
    protected function sendEightBitSMS()
    {
//        $post_body = $this->eight_bit_sms($username, $password, $eight_bit_msg, $msisdn);
...
//        curl_setopt($ch, CURLOPT_POST, 1);
    ...
    */
    ...
    {
        $tryone = Provider::where('account_id', session('shop_id'))->get();
        if (count($tryone) > 0) {
            $tryone = Provider::where('account_id', session('shop_id'))->where('default', 1)->first();
            if ($tryone) {
                return $tryone;
            } else {
                return false;
